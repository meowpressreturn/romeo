...
public class WorldServiceInitialiser implements IServiceInitialiser {
  ...
  public WorldServiceInitialiser(
      final Logger log,
  ...
  public void init(Set<String> tableNames, Connection connection) {
    if(!tableNames.contains("WORLDS")) {
      final String sql = "CREATE TABLE WORLDS (" 
          ...
          + ");";
      _log.info("Creating WORLDS table");
    ...
    if(!tableNames.contains("WORLDS_HISTORY")) {
      _log.info("Creating WORLDS_HISTORY table");
    ...
    _log.debug("Existing columns in WORLDS table=" + worldColumns);
    _log.debug("Nullable columns in WORLDS table=" + nullableWorldColumns);
    ...
    _log.debug("Existing columns in WORLDS_HISTORY table=" + historyColumns);
  ...
  private void updateWorldColumns(Connection connection, Set<String> tableNames, Set<String> worldColumns,
    ...
    try {
      ...
      if(!worldColumns.contains("WORLDEI")) {
        _log.info("Adding worldEi column to WORLDS table");
      ...
      if(!worldColumns.contains("WORLDRER")) {
        _log.info("Adding worldRer column to WORLDS table");
      ...
      if(worldHasColorColumn && !playersTableExists) {
        ...
        _log.info("Creating temporary table to port ancient owner, color information");
        ...
        if(!worldsHasOwnerColumn) {
          ...
          _log.info("Porting players and colors from 0.1.0 style database with only a color columns in WORLDS");
          ...
          _log.info("Porting players and colors from 0.2.0 - 0.4.x style database with both owner and color column in WORLDS");
          ...
          while(rs.next()) {
            ...
            if(!owners.contains(owner)) { //lets just ignore dupes eh?
              _log.info("Porting " + owner + " to ANCIENT_COLORS with color " + color);
              DbUtils.writeQuery("INSERT INTO ANCIENT_COLORS (owner, color) VALUES (?,?)",new Object[] { owner, color}, connection);
              owners.add(owner);
            }
          }
          ...
          if((Boolean)DbUtils.readSingleValue("SELECT COUNT(*)=0 FROM ANCIENT_COLORS WHERE owner=?", new Object[] { IPlayer.NOBODY }, connection) ) {
            _log.info("Explicitly creating an entry for " + IPlayer.NOBODY + " in ANCIENT_COLORS as none was ported");
      ...
      _log.debug("worldsHistoryEmpty=" + worldsHistoryEmpty + ", worldsNotEmpty=" + worldsNotEmpty
      ...
      if(portHistory) {
        _log.info("Porting history column values from WORLDS to WORLDS_HISTORY");
        ...
        _log.info("Converting empty WORLDS.owner values to " + IPlayer.NOBODY);
        ...
        if(assignOwnersFromAncientColors) {
          ...
          _log.info("Determining world ownership based on colors");
          ResultSet rs = DbUtils.readQuery("SELECT owner, color FROM ANCIENT_COLORS",null,connection);
          while(rs.next()) {
            String owner = rs.getString("owner");
            String color = rs.getString("color");
            _log.info("Setting owner to " + owner + " where color is " + color);
      ...
      if(addedOwner || worldColumns.contains("OWNER")) {
        ...
        _log.info("Dropping owner column from WORLDS table");
      ...
      if(addedFirepower || worldColumns.contains("FIREPOWER")) {
        ...
        //Versions prior to 0.6.3 didn't remove it properly though.
        _log.info("Dropping firepower column from WORLDS table");
      ...
      if(worldColumns.contains("COLOR")) {
        ...
        _log.info("Dropping color column from WORLDS table");
      ...
      if(!worldColumns.contains("SCANNERID")) {
        ...
        _log.info("Adding scannerId column to WORLDS table");
      ...
      if(worldColumns.contains("SCANNER")) {
        ...
        _log.info("Dropping scanner column from WORLDS table");
        DbUtils.writeQuery(sql, null, connection);
      }
      ...
      if(nullableWorldColumns.contains("WORLDX")) {
        _log.info("Making worldX,worldY,worldEi,worldRer columns in WORLDS table not null");
        DbUtils.makeNonNull("WORLDS", "0", connection, "worldX","worldY","worldEi","worldRer");
      }
      if(nullableWorldColumns.contains("NAME")) {
        _log.info("Making name column in WORLDS table not null");
        DbUtils.makeNonNull("WORLDS", "''", connection, "name");
      }
      if(nullableWorldColumns.contains("NOTES")) {
        _log.info("Making notes column in WORLDS table not null");
...
//        DbUtils.writeQuery("ALTER TABLE WORLDS ALTER COLUMN nameLookup SET NOT NULL", null, connection);
  ...
  private void updateWorldsHistoryColumns(Connection connection, Set<String> historyColumns, Set<String> nullableHistoryColumns) {
    try {
      ...
      if(nullableHistoryColumns.contains("OWNER")) {
        _log.info("Making column OWNER NOT NULL");
  ...
  protected void copyScanners(Connection connection) throws SQLException {
    ...
    if(worlds > 0) { //If we have any existing worlds in the old format with a scanner range set then we shall get a list
      ...
      _log.info("Updating scanner data");
      ...
      if(count>0) {
        _log.info("Converted default range to null scannerId for " + count + " rows in WORLDS");
      } else {
        _log.debug("No rows were found in WORLDS with default scanner range to be converted");
      }
      ...
      while(rangesRs.next()) {
        ...
        if(!unitExists) {
          _log.info("Creating implied scanner unit with range " + range);          
      ...
      while(scanners.next()) {
        ...
        _log.info("Using unitId " + scannerId + " for scanner of worlds with range " + scannerRange);
      ...
      if(unported > 0) {
        _log.error("Unable to port scanner for " + unported + " worlds due to lack of compatible scanner units. ("
  ...
  public void reset(Connection connection) {
    _log.debug("Deleting contents of the WORLDS and WORLDS_HISTORY table");
